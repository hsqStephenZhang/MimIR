...
void init_def(py::module_& m) {
    ...
    py::class_<mim::Def, std::unique_ptr<mim::Def, py::nodelete>>(m, "Def")
        .def("type", &mim::Def::type, py::return_value_policy::reference_internal)
        ...
        .def("__repr__", [](const mim::Def& d) {
            std::ostringstream os;
            d.stream(os, 256);
            return "<mim.Def " + os.str() + ">";
        });
...
void init_lit(py::module_& m) {
    ...
    py::class_<mim::Lit, mim::Def>(m, "Lit")
        .def("value", [](const mim::Lit& lit) { return lit.get(); });
