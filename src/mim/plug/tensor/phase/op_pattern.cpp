...
static bool is_zero_idx(const Def* idx) {
    ...
    if (auto app = idx->isa<App>()) {
        if (Axm::isa<affine::constant>(app)) return Lit::isa<u64>(app->arg()) && *Lit::isa<u64>(app->arg()) == 0;
    }
    return false;
}
...
static bool is_broadcast_map(const Def* map) {
    if (auto lam = map->isa<Lam>()) {
        ...
        if (auto tuple = body->isa<Tuple>()) {
            ...
            for (size_t j = 0; j < tuple->num_ops(); ++j) {
                auto op = tuple->op(j);
                if (is_zero_idx(op)) continue;
                if (auto ex = op->isa<Extract>()) {
                    if (ex->tuple() == var) {
                        if (auto idx_lit = Lit::isa<u64>(ex->index())) {
                            if (!first && *idx_lit <= last_idx) return false;
                            last_idx = *idx_lit;
                            first    = false;
                            continue;
                        }
...
const Def* OpPatternAnalysis::rewrite_imm_App(const App* app) {
    ...
    } else if (Axm::isa<tensor::transpose>(app) || Axm::isa<tensor::transpose_2d>(app) || Axm::isa<tensor::reshape>(app)
               || Axm::isa<tensor::repeat>(app)) {
        record_kind(OpPatternKind::kInjective);
    } else if (Axm::isa<tensor::map>(app) || Axm::isa<tensor::unary>(app) || Axm::isa<tensor::binary>(app)
...
std::string OpPatternAnalysis::summarize_map_reduce_aff(const App* mra) {
    ...
    std::string summary
        = std::format("(nis={}, Ro={}, Rr={}, map_out={}, maps=[", nis_lit ? std::to_string(*nis_lit) : "?",
                      Ro_lit ? std::to_string(*Ro_lit) : "?", Rr_lit ? std::to_string(*Rr_lit) : "?", map_tag(map_out));
    if (nis_lit) {
...
OpPatternKind classify_map_reduce(const App* mra) {
    ...
    if (!is_id_map(map_out)) is_elem_wise = false;
    ...
    if (has_injective || !is_elem_wise) return OpPatternKind::kInjective;

    // ElemWise Assimilation Rule from TVM:
    // If at least one input is kElemWise and the rest are kBroadcast, overall is kElemWise.
    if (has_elem_wise) return OpPatternKind::kElemWise;

    if (has_broadcast) return OpPatternKind::kBroadcast;
