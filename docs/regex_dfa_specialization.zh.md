...
closed-table decoder 会在生成 residual code 前删除所有 `lo > hi` 的填充项，因此固定容量不会引入无用的 runtime comparisons。
...
解决方式：测试使用 `k = 2` 并加入 never-match padding；specializer 在静态 decode 时删除这些 padding。长期可以考虑为 DFA constructor 增加更明确的 type annotation/constructor API。
