...
driver = Driver()
driver.log().set_stdout().set(Level.Debug)
...
setattr(World, "call", call)


class MimRegex:
    def __init__(self, driver: Driver, *args):
    ...
    def star(self) -> Self:
        ...
        self.regex.append(self.wrld.call(
            self._star_sym, [self.regex[len(self.regex)-1]]))
        return self
    ...
    def build(self, driver):
        ...
        regex_mem, matched, pos = self.wrld.implicit_app(
            self.regex[0],
            [mem, to_match, self.wrld.lit(
                self.wrld.type_idx(self.wrld.top_nat()), 0)],
        ).projs(3)
        ...
        subprocess.run(["clang", "regex.ll", "-o", "regex.so",
                       "-Wno-override-module", "-shared"], check=True)
        self.exec()
