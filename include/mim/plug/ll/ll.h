    ...
    if (auto lit = def->isa<Lit>()) {
        ...
        if (is_simd(tuple->type()) && !Axm::isa<mem::M>(tuple->type())) {
            ...
            if (auto li = Lit::isa(index))
                return bb.assign(name, "extractelement {} {}, i64 {}", t_tup, v_tup, *li);
            auto [v_i, t_i] = emit_gep_index(index);
            return bb.assign(name, "extractelement {} {}, {} {}", t_tup, v_tup, t_i, v_i);
        }
        ...
        if (auto idx = Lit::isa(insert->index())) {
            ...
            if (is_simd(insert->tuple()->type()))
                ...
                return bb.assign(name, "insertelement {} {}, {} {}, i32 {}", t_tup, v_tup, t_val, v_val, *idx);
            else
